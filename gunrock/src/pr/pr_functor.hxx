...
namespace pr {
    ...
    struct pr_functor_t {
        static __device__ __forceinline__ bool cond_filter_upsweep(int idx, pr_problem_t::data_slice_t *data, int iteration) {
		...
		return ((data->d_degrees[idx]>0)? data->d_current_ranks[idx]/data->d_degrees[idx]:0);
        }
    };
